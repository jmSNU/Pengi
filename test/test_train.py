...
class MUSDB18Dataset(Dataset):
    def __init__(self, musdb_root: str, subset: str = "train", duration: float = None, sample_rate: int = 44100):
    ...
    def __getitem__(self, idx):
        ...
        if self.duration:
            ...
            track.chunk_start = 0  
            track.chunk_random = False

        mixture = torch.tensor(track.audio.sum(axis=1), dtype=torch.float32)  
        stems = torch.tensor(
            np.array([
                track.targets["vocals"].audio.sum(axis=1),
                track.targets["drums"].audio.sum(axis=1),
                track.targets["bass"].audio.sum(axis=1),
                track.targets["other"].audio.sum(axis=1)
            ]), dtype=torch.float32)
...
class MSSTrainer:
    def __init__(self, model, preprocess_text, prompts, train_args):
        ...
        musdb_root = "/home/jm/machine_listening_ws/Pengi/dataset/musdb18_wav"
        self.dataset = MUSDB18Dataset(
            musdb_root,
            subset="train", 
